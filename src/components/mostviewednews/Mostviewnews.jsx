...
const MostNewsViewed = () => {
  const [newsContentPage, setNewsContentPage] = useState([]);
  const [moreCardPage, setMoreCardPage] = useState(1);
  const [notNews, setNotNews] = useState([]);
  const [vital, setVital] = useState([]);
  const [vitalPage, setVitalPage] = useState(1);
  const [vitalNamePage, setVitalNamePage] = useState([]);
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .then((result) => {
        setNewsContentPage(result.object.content);
      })
  ...
  const handleMoreCard = () => {
    ...
    fetch(
      `http://165.232.85.45:1988/koinot/news/maxSee?page=${moreCardPage}&size=10&sort=see`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
        setMoreCardPage(moreCardPage + 1);
        setNewsContentPage([...newsContentPage, ...result.object.content]);
  ...
  const handleinformations = () => {
    ...
    fetch(
      `http://165.232.85.45:1988/koinot/news/newsStatus?page=${vitalPage}&size=10&sort=id&status=IMPORTANT`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
        ...
        setVitalNamePage(result.object.content.length !== 0);
      })
      .catch((error) => console.log("error", error));
  };
    ...
    <>
      <div className="container">
        <div className="nav_map">
          ...
          <span className="this_page">Eng ko'p o'qilganlar</span>
        </div>
      </div>
      <main>
        <div className="container_news_main">
          {newsContentPage.length > 0 ? (
            <section className="mainContent_tag">
              ...
              <div className="grid-container">
                {newsContentPage.slice(1).map((cat) => (
