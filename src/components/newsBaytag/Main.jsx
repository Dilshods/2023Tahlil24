...
const Main = () => {
  const [newsTagContent, setNewsTagContent] = useState([]);
  const [moreCard, setMoreCard] = useState(1);
  const [vitalTag, setVitalTag] = useState([]);
  const [vitalTagPage, setVitalTagPage] = useState(1);
  const [tagNews, setTagNews] = useState([]);
  const [vitalNumber, setVitalNumber] = useState([]);
  ...
  useEffect(() => {
    ...
    fetch(
      `http://165.232.85.45:1988/koinot/news/byTagId/${tagId}?page=0&size=10&sort=id`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
        setNewsTagContent(result.object.content);
      })
  ...
  const handleMoreCard = () => {
    ...
    fetch(
      `http://165.232.85.45:1988/koinot/news/byTagId/1?page=${moreCard}&size=10&sort=id`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
        setMoreCard(moreCard + 1);
        setNewsTagContent([...newsTagContent, ...result.object.content]);
        setTagNews(result.object.content.length !== 0);
      })
      .catch((error) => console.log("error", error));
  };
  ...
  useEffect(() => {
    ...
    fetch(
      `
      http://165.232.85.45:1988/koinot/news/byCategoryId/1?page=0&size=10&sort=id`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
        setVitalTag(result.object.content);
      })
      .catch((error) => console.log("error", error));
  }, [id]);
  ...
  const handleinformations = () => {
    ...
    fetch(
      `
      http://165.232.85.45:1988/koinot/news/byCategoryId/1?page=${vitalTagPage}&size=10&sort=id`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
        setVitalTagPage(vitalTagPage + 1);
        setVitalTag([...vitalTag, ...result.object.content]);
        setVitalNumber(result.object.content.length !== 0);
      })
      .catch((error) => console.log("error", error));
  };
    ...
    <>
      <main>
        <div className="container_news_main">
          {newsTagContent.length > 0 ? (
            <section className="mainContent_tag">
              {newsTagContent.slice(0, 1).map((cat) => {
                return (
              ...
              <div className="grid-container">
                {newsTagContent.slice(1).map((cate) => (
                  <div className="card_normal_wrap " key={cate.id}>
                    <a href={`/news/${cate.id}`} style={{ width: "100%" }}>
                      <img
                        src={cate.attachment.link}
                        className="card_img"
                        width="277"
                        alt=""
                      />
                    ...
                    <div className="card_info">
                      <a href={`/news/${cate.id}`}>
                        <h4 className="category_news">
                          {cate.category.textUz}
                        </h4>
                      </a>
                      ...
                      <span className="date_hours_event">
                        {" "}
                        {cate.eventTime}
                      </span>
                    </div>
                    <a href={`/news/${cate.id}`}>
                      <h4 className="card_title"> {cate.titleUz}</h4>
                    </a>
                  </div>
                ))}
              </div>
          ...
          <section className="sideContent">
            ...
            {vitalTag.map((vit) => {
              return (
                <div className="top__sider-items" key={vit.id}>
                    ...
                    <div className="sc-hKgJUU bUdCrb">
                      <div className="card__sider-inners">
                        <div className="sider__info">
                          ...
                          <div className="sider__date date">
